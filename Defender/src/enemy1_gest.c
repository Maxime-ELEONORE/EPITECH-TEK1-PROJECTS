...
{
    sfVector2f pos = sfSprite_getPosition(game->enemy->enemy1[i].sprite);

    if (pos.x == 140 && pos.y > 630) {
        game->enemy->enemy1[i].rect.top = 48 * 3;
        sfSprite_move(game->enemy->enemy1[i].sprite, (sfVector2f){0, -5});
    } else if (pos.y == 630 && pos.x < 390) {
        game->enemy->enemy1[i].rect.top = 48 * 2;
        sfSprite_move(game->enemy->enemy1[i].sprite, (sfVector2f){5, 0});
    }
}
...
void enemy1_trav4(game_t *game, int i)
{
    ...
    if (pos.x == 635 && pos.y < 865) {
        game->enemy->enemy1[i].rect.top = 0;
        sfSprite_move(game->enemy->enemy1[i].sprite, (sfVector2f){0, 5});
    } else if (pos.y == 865 && pos.x > 140) {
        game->enemy->enemy1[i].rect.top = 48;
        sfSprite_move(game->enemy->enemy1[i].sprite, (sfVector2f){-5, 0});
    } else
        enemy1_trav5(game, i);
}
